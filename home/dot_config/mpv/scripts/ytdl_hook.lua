...
local function option_was_set(name)
    return mp.get_property_bool("option-info/" .. name ..
                                    "/set-from-commandline", false)
end
...
local function append_rtmp_prop(props, name, value)
    if not name or not value then return props end
...
local function edl_track_joined(fragments, protocol, is_live, base)
    ...
    if (protocol == "http_dash_segments") and not fragments[1].duration and
        not is_live then
        -- assume MP4 DASH initialization segment
        table.insert(parts, "!mp4_dash,init=" ..
                         edl_escape(join_url(base, fragments[1])))
        offset = 2
...
local function add_single_video(json)
    ...
    if not (json["requested_formats"] == nil) then
        for _, track in pairs(json.requested_formats) do
            ...
            if track.acodec and track.acodec ~= "none" then
                -- audio track
                mp.commandv("audio-add", edl_track or track.url, "auto",
    ...
    if (json.protocol == "rtmp") then
        ...
        rtmp_prop = append_rtmp_prop(rtmp_prop, "rtmp_swfverify",
                                     json.player_url)
        rtmp_prop = append_rtmp_prop(rtmp_prop, "rtmp_swfurl", json.player_url)
...
mp.add_hook("on_load", 10, function()
    ...
    if (url:find("ytdl://") == 1) or
        ...
        -- strip ytdl://
        if (url:find("ytdl://") == 1) then url = url:sub(8) end
        ...
        local command = {
            ...
            "ass/srt/best", "--no-playlist"
        }

        -- Checks if video option is "no", change format accordingly,
        -- but only if user didn't explicitly set one
        if (mp.get_property("options/vid") == "no") and
            not option_was_set("ytdl-format") then

            format = "bestaudio/best"
            msg.verbose("Video disabled. Only using audio")
        end

        if (format == "") then format = "bestvideo+bestaudio/best" end
        ...
        if (allsubs == true) then table.insert(command, "--all-subs") end
        ...
        if not (json["direct"] == nil) and (json["direct"] == true) then
            ...
            return
        elseif not (json["_type"] == nil) and
            ((json["_type"] == "playlist") or (json["_type"] == "multi_video")) then
            -- a playlist
            ...
            if (not (json.entries[1]["_type"] == "url_transparent")) and
                (not (json.entries[1]["webpage_url"] == nil) and
                    (json.entries[1]["webpage_url"] == json["webpage_url"])) and
                not (json.entries[1].url == nil) then
                ...
                if not (entry_wsubs == nil) and
                    ...
                    for j, req in pairs(json.entries[entry_wsubs]
                                            .requested_subtitles) do
                        local subfile = "edl://"
                        for i, entry in pairs(json.entries) do
                            if not (entry.requested_subtitles == nil) and
                                ...
                                subfile = subfile ..
                                              edl_escape(
                                                  entry.requested_subtitles[j]
                                                      .url)
                            else
                                subfile = subfile ..
                                              edl_escape("memory://WEBVTT")
                            end
            ...
            elseif (not (json.entries[1]["_type"] == "url_transparent")) and
                ...
                for i, entry in pairs(json.entries) do
                    ...
                    -- we want
                    if not (json.entries[1]["_type"] == "url_transparent") and
                        not (entry["webpage_url"] == nil) then
