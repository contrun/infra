...
local function set_ASS(b)
    if not o.use_ass or o.persistent_overlay then return "" end
    return b and ass_start or ass_stop
end

local function no_ASS(t) return set_ASS(false) .. t .. set_ASS(true) end

local function b(t) return o.b1 .. t .. o.b0 end

local function it(t) return o.it1 .. t .. o.it0 end
...
local function has_vo_window() return mp.get_property("vo-configured") == "yes" end

local function has_ansi()
    local is_windows = type(package) == 'table' and type(package.config) ==
                           'string' and package.config:sub(1, 1) == '\\'
    if is_windows then return os.getenv("ANSICON") end
    return true
end
...
local function generate_graph(values, i, len, v_max, v_avg, scale, x_tics)
    ...
    if v_avg then scale = min(scale, v_max / (2 * v_avg)) end

    local s = {
        format("m 0 0 n %f %f l ", x,
               y_max - (y_max * values[i] / v_max * scale))
    }
    i = ((i - 2) % len) + 1
    ...
    return format(
               ...
               o.prefix_sep, y_offset, bg_box, o.plot_color, table.concat(s),
               text_style())
end
...
local function append(s, str, attr)
    ...
    s[#s + 1] = format("%s%s%s%s%s%s", attr.nl, attr.indent, attr.prefix,
                       attr.prefix_sep, no_ASS(str), attr.suffix)
    return true
end
...
local function append_perfdata(s, dedicated_page)
    ...
    local target_fps = ds and mp.get_property_number("display-fps", 0) or
                           mp.get_property_number(compat("container-fps"), 0)
    ...
    s[#s + 1] = format("%s%s%s%s{\\fs%s}%s{\\fs%s}",
                       ...
                       dedicated_page and "" or o.indent, b("Frame Timings:"),
                       o.prefix_sep, o.font_size * 0.66,
        ...
        if dedicated_page then
            ...
            for _, pass in ipairs(data) do
                s[#s + 1] = format(f, o.nl, o.indent, o.indent, o.font_mono,
                                   pp(pass["last"]), pp(pass["avg"]),
                                   pp(pass["peak"]),
                                   o.prefix_sep .. o.prefix_sep,
                                   p(pass["last"], last_s[frame]), o.font,
                                   o.prefix_sep, o.prefix_sep, pass["desc"])
            ...
            s[#s + 1] = format(f, o.nl, o.indent, o.indent, o.font_mono,
                               pp(last_s[frame]), pp(avg_s[frame]),
                               pp(peak_s[frame]), "", "", o.font, o.prefix_sep,
                               o.prefix_sep, b("Total"))
        else
...
local function append_display_sync(s)
    ...
    local vspeed =
        append_property(s, "video-speed-correction", {prefix = "DS:"})
    if vspeed then
    ...
    if not display_timer.oneshot and (o.plot_vsync_ratio or o.plot_vsync_jitter) and
        o.use_ass then
        local ratio_graph = ""
        local jitter_graph = ""
        ...
        if o.plot_vsync_jitter then
            jitter_graph = generate_graph(vsjitter_buf, vsjitter_buf.pos,
                                          vsjitter_buf.len, vsjitter_buf.max,
                                          nil, 0.8, 1)
        end
        ...
        local vratio = append_property(s, "vsync-ratio",
                                       {prefix = "VSync Ratio:"})
        append_property(s, "vsync-jitter",
                        {prefix = "VSync Jitter:", nl = "" or o.nl})
    end
end
...
local function add_file(s)
    ...
    if not (mp.get_property_osd("filename") ==
        mp.get_property_osd("media-title")) then
    ...
    if stream_cache + demuxer_cache + demuxer_secs > 0 then
        append(s, utils.format_bytes_humanized(stream_cache + demuxer_cache),
               {prefix = "Total Cache:"})
        ...
        if speed > 0 then
            append(s, utils.format_bytes_humanized(speed) .. "/s", {
                prefix = "Speed:",
                nl = "",
                indent = o.prefix_sep,
                no_prefix_markup = true
...
local function add_video(s)
    ...
    if not r then r = mp.get_property_native("video-out-params") end
    ...
    if append_property(s, compat("decoder-frame-drop-count"),
        ...
        append_property(s, compat("frame-drop-count"),
                        {suffix = " (output)", nl = "", indent = ""})
    end
    if append_property(s, "display-fps",
                       {prefix = "Display FPS:", suffix = " (specified)"}) then
    ...
    if append_property(s, compat("container-fps"),
                       {prefix = "FPS:", suffix = " (specified)"}) then
    ...
    local cmat = append(s, r["colormatrix"],
    ...
    append_property(s, "packet-video-bitrate",
...
local function add_audio(s)
    ...
    if not r then r = mp.get_property_native("audio-out-params") end
    if not r then return end
    ...
    append_property(s, "packet-audio-bitrate",
                    {prefix = "Bitrate:", suffix = " kbps"})
...
local function filter_stats() return "coming soon" end
...
local function print_page(page)
    if o.persistent_overlay then
        ...
        mp.osd_message(pages[page].f(), display_timer.oneshot and o.duration or
                           o.redraw_delay + 1)
    end
end
...
local function remove_page_bindings()
    for k, _ in pairs(pages) do mp.remove_key_binding(k) end
end
...
mp.add_key_binding(o.key_oneshot, "display-stats",
                   function() process_key_binding(true) end, {repeatable = true})

-- Toggling key binding
mp.add_key_binding(o.key_toggle, "display-stats-toggle",
