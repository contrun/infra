...
function shouldIgnore(subtext)
    if ignorePattern and subtext and subtext ~= "" then
        ...
        if st:find(subPattern) then return true end
    else
        return false
    end
end
...
function restore_normalspeed()
    ...
    if (aid ~= nil and aid ~= mp.get_property("aid")) then
...
function speed_transition(_, sub)
    if sub ~= nil and shouldIgnore(sub) then sub = "" end
    if state == 0 then
        if sub == "" then
            ...
            if shouldspeedup or (skipmode and not firstskip) then
                ...
                if skipmode and not temp_disable_skipmode and
                    mp.get_property("pause") == "no" then
                    if firstskip or skipdelay == 0 then
                        mp.commandv("no-osd", "seek", skipval(), "relative",
                                    "exact")
                        firstskip = false
                        ...
                        mp.add_timeout(skipdelay, function()
                            ...
                            if mp.get_property("pause") == "no" then
                                ...
                                if shouldskip or nextsub > leadin then
                                    ...
                                    if tSkip > minSkip and
                                        (currentSub == "" or
                                            shouldIgnore(currentSub)) then
                                        mp.commandv("no-osd", "seek", tSkip,
                                                    "relative", "exact")
                                    else
                                        firstskip = true
                                    end
...
mp.add_key_binding("alt++", "increase_speedup",
                   function() change_speedup(0.1) end, {repeatable = true})
mp.add_key_binding("alt+-", "decrease_speedup",
                   function() change_speedup(-0.1) end, {repeatable = true})
mp.add_key_binding("alt+0", "increase_leadin",
                   function() change_leadin(0.25) end)
mp.add_key_binding("alt+9", "decrease_leadin",
                   function() change_leadin(-0.25) end)
mp.add_key_binding("alt+8", "increase_lookahead",
                   function() change_lookAhead(0.25) end)
mp.add_key_binding("alt+7", "decrease_lookahead",
                   function() change_lookAhead(-0.25) end)
