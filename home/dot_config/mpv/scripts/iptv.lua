...
local chars = {}
for i = string.byte('a'), string.byte('z') do table.insert(chars, i) end
for i = string.byte('A'), string.byte('Z') do table.insert(chars, i) end
for i = string.byte('0'), string.byte('9') do table.insert(chars, i) end
for _, v in ipairs({
    ',', '^', '$', '(', ')', '%', '.', '[', ']', '*', '+', '-', '?', '`', "'",
    ";"
}) do table.insert(chars, string.byte(v)) end
...
local playlister = {
    ...
    init = function(self)
        ...
        if self.pls[1] then self.pls[1].current = false end
        if favorites and #favorites > 0 then self:sortfavs() end
        pattern = ""
    ...
    show = function(self)
        ...
        while self.plsfiltered[i] and i <= self.wndstart + window - 1 do
            ...
            msg =
                msg .. prefix .. (self.pls[self.plsfiltered[i]].title or "") ..
                    "\n"
            i = i + 1
        end
    ...
    sortfavs = function(self)
        ...
        for i = 1, #nonfavs do favs[#favs + 1] = nonfavs[i] end
        self.pls = favs
    end,
    ...
    play = function(self)
        mp.commandv("loadfile", self.pls[self.plsfiltered[self.wndstart +
                        self.cursor]].filename)
        if self.plspos then self.pls[self.plspos].current = false end
...
function add_bindings()
    ...
    for i, v in ipairs(cyr_chars) do
        mp.add_forced_key_binding(v, 'search' .. i + 1000, typing(v),
                                  "repeatable")
    end
end
...
function tablekeys(t)
    local result = {}
    for i, v in ipairs(t) do table.insert(result, i) end
    return result
end
...
function mylower(s)
    local res, n = string.gsub(s, utf8_char,
                               function(c) return utf8_uc_lc[c] end)
    return res
end

function myupper(s)
    local res, n = string.gsub(s, utf8_char,
                               function(c) return utf8_lc_uc[c] end)
    return res
end
