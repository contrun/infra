...
rec {
  ...
  autossh =
    ...
    let
      ...
      getPort =
        host: server: n:
        let
          hash = builtins.hashString "sha512" "${server}->${host}->${builtins.toString n}";
  ...
  getAttrOrNull =
    attrset: path:
    builtins.foldl' (acc: x: if acc ? ${x} then acc.${x} else null) attrset (lib.splitString "." path);

  mkIfAttrExists =
    attrset: path:
    let
      r = getAttrOrNull attrset path;
    in
    lib.mkIf (r != null) r;
}
