...
function isempty(v)
    return (v == false) or (v == nil) or (v == "") or (v == 0) or
...
path_utils.expanduser = function(path)
    ...
    if head == "~~desktop" then
        head = ON_WINDOWS and
                   path_utils.join(os.getenv('USERPROFILE'), 'Desktop') or
                   os.getenv('HOME')
    ...
    elseif head == "~~" then
        ...
        if mpv_config_dir then
            ...
            msg.warn(
                'Could not find mpv config directory (using mp.find_config_file), using temp instead')
            head = ON_WINDOWS and os.getenv('TEMP') or
                       (os.getenv('TMP') or '/tmp/')
        end
    elseif head == "~" then
...
if ON_WINDOWS then
    ...
    path_utils.normpath = function(path)
        if path:find('\\\\.\\', nil, true) == 1 or
            path:find('\\\\?\\', nil, true) == 1 then
        ...
        if prefix == '' and #comps == 0 then comps[1] = curdir end
    ...
    path_utils.relpath = function(path, start)
        ...
        for j = 1, math.min(#start_list, #path_list) do
            if path_utils.normcase(start_list[j]) ~=
                path_utils.normcase(path_list[j]) then break end
            i = j + 1
        end
        ...
        if #rel_list == 0 then return curdir end
    ...
    path_utils.normpath = function(path)
        ...
        if initial_slashes then path = sep:rep(initial_slashes) .. path end
    ...
    path_utils.relpath = function(path, start)
        ...
        local i = 1
        for j = 1, math.min(#start_list, #path_list) do
            if start_list[j] ~= path_list[j] then break end
            i = j + 1
        end
...
function busy_wait(seconds)
    ...
    local cycles = 0
    while target > mp.get_time() do cycles = cycles + 1 end
    return cycles
end

-- Removes all keys from a table, without destroying the reference to it
function clear_table(target)
    for key, value in pairs(target) do target[key] = nil end
end
...
function move_file(source_path, target_path)
    ...
    if ON_WINDOWS then
        cmd = {
            cancellable = false,
            args = {'cmd', '/c', 'move', '/Y', source_path, target_path}
        }
        utils.subprocess(cmd)
    else
...
function check_pid(pid)
    ...
    if ON_WINDOWS then
        cmd = {
            cancellable = false,
            args = {'tasklist', '/FI', ('PID eq %d'):format(pid)}
        }
        ...
        cmd = {
            cancellable = false,
            args = {'sh', '-c', ('kill -0 %d 2>/dev/null'):format(pid)}
        }
        r = utils.subprocess(cmd)
        return r.status == 0
    end
...
function kill_pid(pid)
    ...
    if ON_WINDOWS then
        cmd = {
            cancellable = false,
            args = {'taskkill', '/F', '/PID', tostring(pid)}
        }
    else
...
function format_time(seconds, sep, decimals)
    ...
    local second_format = string.format("%%0%d.%df", 2 +
                                            (decimals > 0 and decimals + 1 or 0),
                                        decimals)

    return string.format("%02d" .. sep .. "%02d" .. sep .. second_format, h, m,
                         s)
end
...
function format_time_hms(seconds, sep, decimals, force_full)
    ...
    if force_full or h > 0 then
        return string.format("%dh" .. sep .. "%dm" .. sep .. "%." ..
                                 tostring(decimals) .. "fs", h, m, s)
    elseif m > 0 then
        return string.format("%dm" .. sep .. "%." .. tostring(decimals) .. "fs",
                             m, s)
    else
...
function extend_table(target, source)
    for i, v in pairs(source) do table.insert(target, v) end
end
...
function round_rect(ass, x0, y0, x1, y1, rtl, rtr, rbr, rbl)
    ...
    if rtr > 0 then
        ass:bezier_curve(x1 - rtr + rtr * c, y0, x1, y0 + rtr - rtr * c, x1,
                         y0 + rtr) -- top right corner
    end
    ass:line_to(x1, y1 - rbr) -- right line
    if rbr > 0 then
        ass:bezier_curve(x1, y1 - rbr + rbr * c, x1 - rbr + rbr * c, y1,
                         x1 - rbr, y1) -- bottom right corner
    end
    ass:line_to(x0 + rbl, y1) -- bottom line
    if rbl > 0 then
        ass:bezier_curve(x0 + rbl - rbl * c, y1, x0, y1 - rbl + rbl * c, x0,
                         y1 - rbl) -- bottom left corner
    end
    ass:line_to(x0, y0 + rtl) -- left line
    if rtl > 0 then
        ass:bezier_curve(x0, y0 + rtl - rtl * c, x0 + rtl - rtl * c, y0,
                         x0 + rtl, y0) -- top left corner
    end
end
--[[
...
setmetatable(OptionParser, {__call = function(cls, ...) return cls.new(...) end})
...
function OptionParser.new(identifier)
    ...
    self.OVERRIDE_START =
        "# Script-saved overrides below this line. Edits will be lost!"
                ...
                return setmetatable({}, {
                    ...
                    __newindex = function(t, sub_key, value)
                        ...
                        if sub_option and sub_option.type ~= "table" then
                            self.active_profile.values[key .. "." .. sub_key] =
                                value
                        end
                    end
                })
    ...
    self.values = setmetatable({}, {
        __index = function(t, key) return get_value_or_mapper(key) end,
    ...
    self.defaults = setmetatable({}, {
        __index = function(t, key) return get_default_or_mapper(key) end
    })
    ...
    mp.add_timeout(0, function()
        ...
        local example_dump_filename = mp.get_opt(self.identifier ..
                                                     "-example-config")
        ...
        if explain_config then self:explain_options() end

        if (example_dump_filename or explain_config) and
            mp.get_property_native("options/idle") then
            msg.info("Exiting.")
            mp.commandv("quit")
        end
    end)
...
function OptionParser:activate_profile(profile_name)
    ...
    if chosen_profile then self.active_profile = chosen_profile end
...
function OptionParser:add_option(key, default, description, pad_before)
    ...
    if option_type == "table" then
        if default._array then option_type = "array" end
        default._array = nil
    end
...
function OptionParser:get_profile(profile_name)
    for i, profile in ipairs(self.profiles) do
        if profile.name == profile_name then return profile end
    end
end
...
function OptionParser:load_options()
    ...
    local trim = function(text) return (text:gsub("^%s*(.-)%s*$", "%1")) end
    ...
    while line_index < total_lines + 1 do
        ...
        elseif profile_name then
            current_profile = self:get_profile(profile_name) or
                                  self:create_profile(profile_name, true)
            ...
            if key then
                ...
                local option = self.options[key]
                if not option then
                    msg.warn(("%s:%d ignoring unknown key '%s'"):format(
                                 self.config_file, line_index, key))
                elseif option.type == "table" then
                    msg.warn(
                        ("%s:%d ignoring value for table-option %s"):format(
                            self.config_file, line_index, key))
                else
                    ...
                    local parsed_value =
                        self:string_to_value(option.type, value)

                    if parsed_value == nil then
                        msg.error(
                            ("%s:%d error parsing value '%s' for key '%s' (as %s)"):format(
                                self.config_file, line_index, value, key,
                                option.type))
                    else
...
function OptionParser:save_options()
    ...
    local add_linebreak = function()
        if out_lines[#out_lines] ~= '' then table.insert(out_lines, '') end
    end
    ...
    for profile_index, profile in ipairs(profiles) do
        ...
        for option_index, option in ipairs(self.options_list) do
            ...
            if option.key then
                if (option_value ~= option_loaded) then
                    table.insert(profile_override_lines, ('%s=%s'):format(
                                     option.key,
                                     self:value_to_string(option_value)))
                end
            end
        end

        if (#profile.config_lines > 0 or #profile_override_lines > 0) and
            profile ~= self.default_profile then
        ...
        if #profile_override_lines > 0 then
            ...
            for override_line_index, override_line in ipairs(
                                                          profile_override_lines) do
...
function OptionParser:get_default_config_lines()
    ...
    for option_index, option in ipairs(self.options_list) do
        if option.pad_before then table.insert(example_config_lines, '') end
        ...
        if option.key and option.type ~= "table" then
            table.insert(example_config_lines, ('%s=%s'):format(option.key,
...
setmetatable(DisplayState, {__call = function(cls, ...) return cls.new(...) end})
...
function DisplayState:recalculate_bounds(forced)
    ...
    if new_state.video_w and new_state.video_h then
        ...
        if new_state.keepaspect then
            ...
            local video_left, video_right =
                self:_split_scaling(new_state.screen_w, scaled_w,
                                    new_state.video_zoom,
                                    new_state.video_align_x,
                                    new_state.video_pan_x)
            local video_top, video_bottom =
                self:_split_scaling(new_state.screen_h, scaled_h,
                                    new_state.video_zoom,
...
function DisplayState:_aspect_calc_panscan(state)
    ...
    if state.video_unscaled then
        vo_panscan_area = 0
        if state.video_unscaled ~= "downscale-big" or
            ((state.video_w <= state.screen_w) and
                (state.video_h <= state.screen_h)) then
    ...
    local scaled_h =
        math.floor(f_height + vo_panscan_area * state.panscan * f_h)
...
function DisplayState:_split_scaling(dst_size, scaled_src_size, zoom, align, pan)
    ...
    local dst_start = (dst_size - scaled_src_size) * align + pan *
                          scaled_src_size
...
setmetatable(ASSCropper, {__call = function(cls, ...) return cls.new(...) end})
...
function ASSCropper.new(display_state)
    ...
    self._key_binds = {
        {"mouse_move", function() self:update_mouse_position() end}, {
            "mouse_btn0", function(e) self:on_mouse("mouse_btn0", e) end,
            {complex = true}
        }, {
            "shift+mouse_btn0",
            function(e) self:on_mouse("mouse_btn0", e, true) end,
            {complex = true}
        ...
        {"z", function() self:key_event("INVERT") end}, {
            "shift+left", function() self:key_event("NUDGE_LEFT") end,
            {repeatable = true}
        },
        {
            "shift+up", function() self:key_event("NUDGE_UP") end,
            {repeatable = true}
        }, {
            "shift+right", function() self:key_event("NUDGE_RIGHT") end,
            {repeatable = true}
        }, {
            "shift+down", function() self:key_event("NUDGE_DOWN") end,
            {repeatable = true}
        },

        {
            "ctrl+left", function() self:key_event("GROW_LEFT") end,
            {repeatable = true}
        },
        {
            "ctrl+up", function() self:key_event("GROW_TOP") end,
            {repeatable = true}
        }, {
            "ctrl+right", function() self:key_event("SHRINK_LEFT") end,
            {repeatable = true}
        },
        {
            "ctrl+down", function() self:key_event("SHRINK_TOP") end,
            {repeatable = true}
        }, {
            ...
            "ctrl+shift+up", function() self:key_event("SHRINK_BOT") end,
            {repeatable = true}
        }, {
            ...
            "ctrl+shift+down", function() self:key_event("GROW_BOT") end,
            {repeatable = true}
...
function ASSCropper:finalize_crop()
    if self.current_crop ~= nil then
        ...
        if self.options.even_dimensions then
            self.current_crop.w = self.current_crop.w -
                                      (self.current_crop.w % 2)
            self.current_crop.h = self.current_crop.h -
                                      (self.current_crop.h % 2)
        end

        self.current_crop.x1, self.current_crop.x2 = x1,
                                                     x1 + self.current_crop.w
        self.current_crop.y1, self.current_crop.y2 = y1,
                                                     y1 + self.current_crop.h

        self.current_crop[2].x, self.current_crop[2].y = self.current_crop.x2,
                                                         self.current_crop.y2
    end
end
...
function ASSCropper:key_event(name)
    ...
    elseif not self.testing_crop then
        ...
        elseif name == "GUIDES" then
            self.options.guide_type = (self.options.guide_type + 1) %
                                          (self.guide_type_count)
...
function ASSCropper:nudge(keep_size, left, top, right, bottom)
    ...
    local vw, vh = self.display_state.video.width,
                   self.display_state.video.height
...
function ASSCropper:start_testing()
    if not self.testing_crop then
        ...
        local cw = self.current_crop and
                       (self.current_crop[2].x - self.current_crop[1].x) or 0
        local ch = self.current_crop and
                       (self.current_crop[2].y - self.current_crop[1].y) or 0
        ...
        local crop_filter = ('@%s:crop=w=%d:h=%d:x=%d:y=%d'):format(
                                self.crop_label, cw, ch, self.current_crop[1].x,
                                self.current_crop[1].y)
        local ret = mp.commandv('vf', 'add', crop_filter)
        if ret then self.testing_crop = true end
    end
end
...
function ASSCropper:blackframe_check()
    local blackframe_metadata = mp.get_property_native("vf-metadata/" ..
                                                           self.blackframe_label)
    local black_percentage = tonumber(
                                 blackframe_metadata["lavfi.blackframe.pblack"])
...
function ASSCropper:blackframe_start()
    ...
    if not self.detecting_blackframe then
        ...
        local blackframe_filter =
            ('@%s:blackframe=amount=%d:threshold=%d'):format(
                self.blackframe_label, 0, 128)
...
function ASSCropper:toggle_crop_detect()
    if self.detecting_crop then
        ...
        local cropdetect_filter =
            ('@%s:cropdetect=limit=%f:round=2:reset=0'):format(
                self.cropdetect_label, 30 / 255)
...
function ASSCropper:start_crop(options, on_crop, on_cancel)
    ...
    if self.display_state.video_ready then
        ...
        for k, v in pairs(self.default_options) do self.options[k] = v end
        for k, v in pairs(options or {}) do self.options[k] = v end
        ...
        if self.options.auto_invert then self:blackframe_start() end
    end
end
...
function ASSCropper:stop_crop(clear)
    ...
    if clear then self.current_crop = nil end
end
...
function ASSCropper:update_mouse_position()
    ...
    if self.display_state:recalculate_bounds() and
        self.display_state.video_ready then
        -- These are on-video coords.
        local mx, my = self.display_state:screen_to_video(self.mouse_screen.x,
                                                          self.mouse_screen.y)
...
function ASSCropper:get_hitboxes(crop_box)
    ...
    if crop_box == nil then return nil end
    ...
    local mult =
        math.min(self.display_state.scale.x, self.display_state.scale.y)
...
function ASSCropper:on_mouse(button, event, shift_down)
    ...
    if button == "mouse_btn0" and self.active and not self.detecting_crop and
        not self.testing_crop then
        ...
        if mouse_down then -- Mouse pressed
            ...
            local bound_mouse_pos = {
                x = math.max(0, math.min(self.display_state.video.width,
                                         mouse_pos.x)),
                y = math.max(0, math.min(self.display_state.video.height,
                                         mouse_pos.y))
            }
            ...
            elseif self.dragging == 0 then
                ...
                self.crop_ratio = (hitboxes[10][3] - hitboxes[10][1]) /
                                      (hitboxes[10][4] - hitboxes[10][2])
                self.drag_start = mouse_pos

                local handle_pos = self:_get_anchor_positions()[hit] or
                                       {mouse_pos.x, mouse_pos.y}
                self.drag_offset = {
                    mouse_pos.x - handle_pos[1], mouse_pos.y - handle_pos[2]
                }
            ...
            if xy_same(self.current_crop[1], self.current_crop[2]) and
                xy_distance(self.current_crop[1], mouse_pos) < 5 then
...
function ASSCropper:get_render_ass(dim_only)
    ...
    local guide_format = string.format(
                             ...
                             128, line_color, line_color, line_color)
    ...
    if self.current_crop then
        ...
        if self.testing_crop then
            ...
            local fmt_key = function(key, text)
                return string.format("[{\\c&HBEBEBE&}%s{\\c} %s]", key:upper(),
                                     text)
            end

            ass:append(fmt_key("ENTER", "Accept crop") .. " " ..
                           fmt_key("ESC", "Cancel crop") .. '\\N' ..
        ...
        for index, coords in pairs(v_hb) do
            local x1, y1 = self.display_state:video_to_screen(coords[1],
                                                              coords[2])
            local x2, y2 = self.display_state:video_to_screen(coords[3],
                                                              coords[4])
        ...
        ass:new_event()
        ass:append(string.format("{\\iclip(%d,%d,%d,%d)}", s_crop[1], s_crop[2],
                                 s_crop[3], s_crop[4]))
        ...
        local format_dim = string.format(
                               ...
                               self.overlay_lightness, self.overlay_lightness,
                               self.overlay_lightness)
        ass:pos(0, 0)
        ass:draw_start()
        ass:append(format_dim)
        ass:rect_cw(0, 0, self.display_state.screen.width,
                    self.display_state.screen.height)
        ass:draw_stop()
        ...
        if draw_text then
            ...
            ass:pos(ce_x, ce_y)
            -- Text align
            local txt_a = ((ce_x > cs_x) and 3 or 1) +
                              ((ce_y > cs_y) and 0 or 6)
            ass:an(txt_a)
            ass:append("{\\fs20\\shad0\\be0\\bord2}")
            ass:append(string.format("%dx%d", math.abs(ce_x - cs_x),
                                     math.abs(ce_y - cs_y)))
        end

        local box_format = string.format(
                               "{\\1a&HFF&\\3a&H%02X&\\3c&H%02X%02X%02X&\\bord1}",
                               0, line_color, line_color, line_color)
        local handle_hilight_format = string.format(
                                          "{\\1a&H%02X&\\3a&H%02X&\\3c&H%02X%02X%02X&\\bord0}",
                                          230, 0, line_color, line_color,
                                          line_color)
        local handle_drag_format = string.format(
                                       "{\\1a&H%02X&\\3a&H%02X&\\3c&H%02X%02X%02X&\\bord1}",
                                       200, 0, line_color, line_color,
                                       line_color)
        ...
        if self.dragging > 0 and drawn_handle ~= 5 then
            ...
            ass:pos(0, 0)
            ass:draw_start()
            ass:rect_cw(s_hb[drawn_handle][1], s_hb[drawn_handle][2],
                        s_hb[drawn_handle][3], s_hb[drawn_handle][4])
            ass:draw_stop()
        elseif self.dragging == 0 then
            ...
            if hit_index > 0 and hit_index ~= 5 then
                ...
                ass:pos(0, 0)
                ass:draw_start()
                ass:rect_cw(s_hb[hit_index][1], s_hb[hit_index][2],
                            s_hb[hit_index][3], s_hb[hit_index][4])
                ass:draw_stop()
            end
        ...
        if true or draw_text then
            ...
            ass:an(br_align)
            ass:append("{\\fs20\\shad0\\be0\\bord2}")
            ass:append(string.format("%dx%d", v_crop[3] - v_crop[1],
                                     v_crop[4] - v_crop[2]))
    ...
    if self.options.draw_help and not dim_only then
        ...
        ass:pos(self.display_state.screen.width - 5, 5)
        local text_align = 9
        ass:append(string.format("{\\fs%d\\an%d\\bord2}", self.text_size,
                                 text_align))

        local fmt_key = function(key, text)
            return
                string.format("[{\\c&HBEBEBE&}%s{\\c} %s]", key:upper(), text)
        end
        ...
        lines = {
            fmt_key("ENTER", "Accept crop") .. " " ..
                ...
                fmt_key("D", "Autodetect crop") .. " " ..
                fmt_key("T", "Test crop"),
            fmt_key("SHIFT-Drag", "Constrain ratio") .. " " ..
                fmt_key("SHIFT-Arrow", "Nudge"),
            fmt_key("C", crosshair_txt .. " crosshair") .. " " ..
                fmt_key("X", "Cycle guides") .. " " ..
                fmt_key("Z", "Invert color")
        }
...
function PropertyExpander:expand(format_string)
    local comparisons = {
        {
            ...
            '^(..-)$=(.+)$', function(property_value, other_value)
                ...
                return other_value == '' or
                           property_value:sub(-other_value:len()) == other_value
            end
        }, {
            -- Contains
            '^(..-)~=(.+)$', function(property_value, other_value)
    ...
    local substitutor = function(match)
        ...
        local raw_property_value = self.property_source:get_raw_property(
                                       property_name, self.sentinel)
        ...
        if command == '' then
            ...
            return property_exists and
                       self.property_source:get_property(property_name) or
                       self:expand(secondary)
        ...
        elseif command == '#' then
            ...
            return self:_format_time(raw_property_value,
                                     has_colon and secondary or nil)
        ...
        elseif command == '@' then
            ...
            return property_exists and
                       self.property_source:get_property_osd(property_name) or
                       self:expand(secondary)
        end
...
function expand_output_path(cropbox)
    ...
    if propex:expand(option_values.output_template) == test_path then
        ...
        local temporary_screenshot_path = propex:expand(
                                              option_values.output_template)
...
function screenshot(crop)
    ...
    if option_values.create_directories then
        ...
        for i, path in ipairs(paths) do
            ...
            if err then create_directories(path) end
        end
    end
    ...
    if option_values.skip_screenshot_for_images and duration == 0 and
        playback_time == 0 then
        ...
        local previous_png_compression =
            mp.get_property_native("screenshot-png-compression")
        mp.set_property_native("screenshot-png-compression", 0)
        -- Take the screenshot
        mp.commandv("raw", "no-osd", "screenshot-to-file",
                    temporary_screenshot_path)
        -- Return the previous value
        mp.set_property_native("screenshot-png-compression",
                               previous_png_compression)
    ...
    local crop_string = string.format("%d:%d:%d:%d", crop.w, crop.h, crop.x,
                                      crop.y)
    local cmd = {
        args = {
            "mpv", input_path, "--no-config", "--vf=crop=" .. crop_string,
            "--frames=1", "--ovc=" .. option_values.output_format, "-o",
            output_path
        }
    }
...
if option_values.warn_about_template and
    not option_values.output_template:find('%${ext}') then
    msg.warn(
        "Output template missing ${ext}! If this is desired, set warn_about_template=yes in config!")
end
...
if option_values.output_extension == "" then
    ...
    if not extension then
        msg.error(
            "Unrecognized output format '" .. option_values.output_format ..
                "', unable to pick an extension! Bailing!")
        mp.osd_message(
            "mpv_crop_script was unable to choose an extension, check your config",
            3)
    end
...
if option_values.disable_keybind then used_keybind = nil end
