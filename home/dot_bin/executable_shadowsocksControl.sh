...
NAME=Shadowsocks
BIN=ss-local
PREFIX=~/.customized/shadowsocks/
PGREPEX="ss-local.*customized/shadowsocks"
...
SERVERS=$(ls $PREFIX | grep -o "^server[0-9]*conf" | uniq | wc -l)
...
case "$2" in
    ...
    4)
        ...
        ;;
    *)
        HOSTNUMBER=$(shuf -i 1-"$SERVERS" -n 1)
        CONFS=$(ls $PREFIX | grep "^server${HOSTNUMBER}" | uniq | wc -l)
        CONFNUMBER=$(shuf -i 1-"$CONFS" -n 1)
        CONF="server${HOSTNUMBER}conf${CONFNUMBER}.json"
        ;;
esac
RETVAL=0
