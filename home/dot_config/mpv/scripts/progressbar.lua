...
local log = {
    debug = function(format, ...) return msg.debug(format:format(...)) end,
    info = function(format, ...) return msg.info(format:format(...)) end,
    warn = function(format, ...) return msg.warn(format:format(...)) end,
    dump = function(item, ignore)
        ...
        local count = 1
        local tablecount = 1
        local result = {"{ @" .. tostring(tablecount)}
        local seen = {[item] = tablecount}
        local recurse
            ...
            for key, value in pairs(item) do
                if not (key == ignore) then
                    if "table" == type(value) then
                        ...
                        count = count + 1
                        result[count] = space .. tostring(key) .. ": " ..
                                            tostring(value)
                    end
                end
            end
...
local settingsMeta = {
    _reload = function(self)
        for key, value in pairs(self._defaults) do settings[key] = value end
    ...
    _migrate = function(self)
        ...
        mv = function(oldFile, newFile)
            local cmd = {args = {'mv', oldConfig, newConfig}}
            if onWindows then
                local oldfile = oldFile:gsub('/', pathSep)
                newFile = newFile:gsub('/', pathSep)
                cmd = {
                    args = {'cmd', '/Q', '/C', 'move', '/Y', oldfile, newFile}
                }
            end
        ...
        mkdir = function(directory)
            local cmd = {args = {'mkdir', '-p', directory}}
            if onWindows then
        ...
        local settingsDirectories = {'script-opts', 'lua-settings'}
        ...
        if oldConfig and not newConfig then
            ...
            log.info(
                ('Old configuration detected. Attempting to migrate "%s" -> "%s"'):format(
                    oldConfig, newConfig))
            local dirExists = mp.find_config_file(configDir)
            if dirExists and not utils.readdir(configDir) then
                log.warn(
                    ('Configuration migration failed. "%s" exists and does not appear to be a folder'):format(
                        configDir))
                return
            else
                if not dirExists then
                    log.debug(('Attempting to create directory "%s"'):format(
                                  configDir))
                    local res = mkdir(configDir)
                    if res.error or res.status ~= 0 then
                        log.warn(('Making directory "%s" failed.'):format(
                                     configDir))
                        return
                    end
                    ...
                    log.debug(
                        ('Directory "%s" already exists. Continuing.'):format(
                            configDir))
                end
            end
            log.debug(('Attempting to move "%s" -> "%s"'):format(oldConfig,
                                                                 newConfig))
            local res = mv(oldConfig, newConfig)
            if res.error or res.status ~= 0 then
                log.warn(('Moving file "%s" -> "%s" failed.'):format(oldConfig,
                                                                     newConfig))
                return
            end
            if mp.find_config_file(newConfigFile) then
                ...
                return log.warn(
                           ('Cannot find "%s". Migration mysteriously failed?'):format(
                               newConfigFile))
            end
        end
    end,
...
settings['default-style'] =
    [[\fnSource Sans Pro\b1\bord2\shad0\fs30\c&HFC799E&\3c&H2D2D2D&]]
...
do
    ...
    local _base_0 = {
        ...
        remove = function(self, element)
            if element[self] == nil then
                error(
                    "Trying to remove an element that doesn't exist in this stack.")
            end
        ...
        removeSortedList = function(self, elementList)
            if #elementList < 1 then return end
    ...
    removeElementMetadata = function(self, element)
        element[self] = nil
        if self.containmentKey then element[self.containmentKey] = false end
    end
    reindex = function(self, start)
        if start == nil then start = 1 end
        for i = start, #self do (self[i])[self] = i end
    end
    Stack = _class_0
end
local Window
...
do
    ...
    local _base_0 = {
        ...
        containsPoint = function(self, x, y)
            return (x >= self.x) and (x < self.xMax) and (y >= self.y) and
                       (y < self.yMax)
        end
    }
    _base_0.__index = _base_0
...
do
    ...
    local _base_0 = {
        reconfigure = function(self) self.active = false end,
        ...
        activityCheck = function(self, displayRequested)
            if displayRequested == true then return true end
            if not (Mouse.inWindow) then return false end
            if Mouse.dead then return false end
        ...
        update = function(self, displayRequested, clickPending)
            ...
            if clickPending then self:clickHandler() end
            return nowActive
        end
    }
    _base_0.__index = _base_0
...
do
    ...
    self.removeAnimation = function(animation)
        if animation.active then return animationList:remove(animation) end
    end
    self.destroyAnimationStack = function() return animationList:clear() end
    ...
    self.active = function() return #animationList > 0 end
...
do
    ...
    local _base_0 = {
        ...
        addUIElement = function(self, uiElement)
            if uiElement == nil then error('nil UIElement added.') end
            self.uiElements:insert(uiElement)
            return table.insert(self.script, '')
        end,
        removeUIElement = function(self, uiElement)
            if uiElement == nil then error('nil UIElement removed.') end
        ...
        redraw = function(self, forceRedraw)
            ...
            if Window:update() then self:resize() end
            ...
            if self.needsRedraw then
                mp.set_osd_ass(Window.w, Window.h,
                               table.concat(self.script, '\n'))
    ...
    _class_0 = setmetatable({
        __init = function(self)
            ...
            self.needsRedraw = false
            self.updateTimer = mp.add_periodic_timer(settings['redraw-period'],
                                                     (function()
                local _base_1 = self
            ...
            mp.add_key_binding("tab", "request-display", function(event)
                if event.event == "repeat" then return end
                ...
                if event.event == "up" or event.event == "press" then
                    if displayDuration == 0 then
                        ...
                        displayRequestTimer =
                            mp.add_timeout(displayDuration, function()
                                self.displayRequested = false
                            end)
                    end
                end
...
do
    ...
    local _base_0 = {
        update = function(self, now)
            if self.isReversed then
                self.linearProgress = math.max(0, math.min(1,
                                                           self.linearProgress +
                                                               (self.lastUpdate -
                                                                   now) *
                                                               self.durationR))
            ...
            self.value = (1 - progress) * self.initialValue + progress *
                             self.endValue
    ...
    _base_0.__index = _base_0
    _class_0 = setmetatable({
        __init = function(self, initialValue, endValue, duration, updateCb,
                          finishedCb, accel)
...
do
    ...
    local _base_0 = {
        ...
        redraw = function(self) return self.needsUpdate end
    }
    _base_0.__index = _base_0
    _class_0 = setmetatable({
        __init = function(self)
...
do
    ...
    local self = _class_0
    barSize = settings['bar-height-active']
    self.changeBarSize = function(size) barSize = size end
    if _parent_0.__inherited then _parent_0.__inherited(_parent_0, _class_0) end
    BarAccent = _class_0
end
local BarBase
do
    ...
    local _base_0 = {
        ...
        reconfigure = function(self, prefix)
            if prefix == nil then prefix = 'bar-' end
            _class_0.__parent.__base.reconfigure(self)
            ...
            self.line[8] = lineBaseTemplate:format(settings['default-style'],
                                                   settings['bar-default-style'],
                                                   '%s')
        ...
        resize = function(self)
            ...
            self.line[9] =
                ([[m 0 0 l %d 0 %d 1 0 1]]):format(Window.w, Window.w)
            self.needsUpdate = true
        end,
        animate = function(self, value)
            self.line[4] = ([[%g]]):format((self.maxHeight -
                                               self.animationMinHeight) * value +
                                               self.animationMinHeight)
            self.needsUpdate = true
        end,
        redraw = function(self)
            if self.hideInactive ~= hideInactive then
                self.hideInactive = hideInactive
                if not (self.active) then self:animate(0) end
            end
    ...
    _class_0 = setmetatable({
        __init = function(self)
            ...
            self.line = {
                [[{\pos(]], 0, [[)\fscy]], minHeight, [[\fscx]], 0.001, [[]],
                lineBaseTemplate, 0
            }
            _class_0.__parent.__init(self)
...
do
    ...
    local _base_0 = {
        reconfigure = function(self)
            _class_0.__parent.__base.reconfigure(self)
            seekString = ('absolute-percent+%s'):format(
                             settings['seek-precision'])
        ...
        redraw = function(self)
            ...
            if position ~= self.lastPosition or self.needsUpdate then
                ...
                if self.barShift > 0 then
                    local followingEdge =
                        Window.w * position * 1e-2 - self.barShift
                    self.line[7] =
                        ([[\clip(m %g 0 l %g 0 %g %g %g %g)]]):format(
                            followingEdge, Window.w, Window.w, Window.h,
...
do
    ...
    local _base_0 = {
        ...
        redraw = function(self)
            ...
            if self.fileDuration and (self.fileDuration > 0) then
                local barDrawing = {past = {}, future = {}}
                local ranges
                ranges =
                    mp.get_property_native('demuxer-cache-state', {})['seekable-ranges']
                if ranges and (#ranges > 0) then
                    local position = mp.get_property_number('percent-pos', 0)
                    local cacheKeyAggregator = {Window.w, position}
                    ...
                    local progressPosition =
                        mp.get_property_number('percent-pos', 0) * Window.w *
                            0.01
                    for _index_0 = 1, #ranges do
                        local _des_0 = ranges[_index_0]
                        local rangeStart, rangeEnd
                        ...
                        elseif rangeStart > progressPosition then
                            ...
                            local rectPast =
                                ('m %g 0 l %g 1 %g 1 %g 0'):format(rangeStart,
                                                                   ...
                                                                   progressPosition)
                            local rectFuture =
                                ('m %g 0 l %g 1 %g 1 %g 0'):format(0, 0,
                                                                   rangeEnd,
                                                                   rangeEnd)
    ...
    _class_0 = setmetatable({
        __init = function(self)
            ...
            return mp.observe_property('duration', 'number',
                                       function(name, value)
...
do
    ...
    local _base_0 = {
        stringify = function(self) return table.concat(self.line) end,
        resize = function(self)
            self.line[2] = ('%d,%d'):format(
                               math.floor(self.position * Window.w), Window.h)
        end,
    ...
    _class_0 = setmetatable({
        __init = function(self, position, minWidth, minHeight)
            ...
            self.line = {
                ...
                [[)\fscx]], minWidth, [[\fscy]], minHeight, beforeStyle,
                '}m 0 0 l 1 0 1 1 0 1\n'
            }
            self.passed = false
        end,
...
do
    ...
    local _base_0 = {
        createMarkers = function(self)
            ...
            local markerHeight = self.active and maxHeight * maxHeightFrac or
                                     BarBase.instantiatedBars[1]
                                         .animationMinHeight
            local markerWidth = self.active and maxWidth or minWidth
            for _index_0 = 1, #chapters do
                local chapter = chapters[_index_0]
                local marker = ChapterMarker(chapter.time / totalTime,
        ...
        animate = function(self, value)
            ...
            local height = (maxHeight * maxHeightFrac -
        ...
        redraw = function(self)
            _class_0.__parent.__base.redraw(self)
            local currentPosition = mp.get_property_number('percent-pos', 0) *
                                        0.01
            local update = false
            for i, marker in ipairs(self.markers) do
...
do
    ...
    local _base_0 = {
        reconfigure = function(self)
            ...
            self.line[2] = ('%g,%g'):format(self.position,
                                            self.yPos - bottomMargin)
            self.line[3] = ([[)\an1%s%s}]]):format(settings['default-style'],
                                                   settings['elapsed-style'])
            self.animation = Animation(offscreenPos,
        ...
        redraw = function(self)
            if self.active then
                ...
                if timeElapsed ~= self.lastTime then
                    local update = true
                    self.line[4] = ('%d:%02d:%02d'):format(math.floor(
                                                               timeElapsed /
                                                                   3600),
                                                           math.floor(
                                                               (timeElapsed / 60) %
    ...
    _class_0 = setmetatable({
        __init = function(self)
            ...
            local offscreenPos = settings['elapsed-offscreen-pos']
            self.line = {
                [[{\pos(]], ([[%g,0]]):format(offscreenPos),
                ([[)\an1%s%s}]]):format(settings['default-style'],
                                        settings['elapsed-style']), [[????]]
            }
            self.lastTime = -1
...
do
    ...
    local _base_0 = {
        reconfigure = function(self)
            ...
            self.line[2] = ('%g,%g'):format(self.position,
                                            self.yPos - bottomMargin)
            self.line[3] = ([[)\an3%s%s}]]):format(settings['default-style'],
                                                   settings['remaining-style'])
            self.animation = Animation(offscreenPos,
        ...
        redraw = function(self)
            if self.active then
                _class_0.__parent.__base.redraw(self)
                local timeRemaining = math.floor(
                                          mp.get_property_number(
                                              'playtime-remaining', 0))
    ...
    _class_0 = setmetatable({
        __init = function(self)
            ...
            self.line = {
                [[{\pos(]], ([[%g,0]]):format(offscreenPos),
                ([[)\an3%s%s}]]):format(settings['default-style'],
                                        settings['remaining-style']), [[????]]
            }
            self.lastTime = -1
            self.position = offscreenPos
...
do
    ...
    local _base_0 = {
        reconfigure = function(self)
            ...
            self.line[2] = ('%g,%g'):format(
                               math.min(Window.w - rightMargin,
                                        math.max(leftMargin, Mouse.x)),
                               self.position)
            self.line[1] = ([[{%s%s\pos(]]):format(settings['default-style'],
                                                   settings['hover-time-style'])
            ...
            if self.active then
                ...
                if Mouse.x ~= self.lastX then
                    self.line[2] = ("%g,%g"):format(
                                       ...
                                       self.position)
                    self.lastX = Mouse.x
                    local hoverTime = mp.get_property_number('duration', 0) *
                                          Mouse.x / Window.w
    ...
    _class_0 = setmetatable({
        __init = function(self)
            _class_0.__parent.__init(self)
            self.line = {
                ([[{%s%s\pos(]]):format(settings['default-style'],
                                        settings['hover-time-style']),
                [[-100,0]], [[)\an2}]], [[????]]
            }
            self.lastTime = 0
...
do
    ...
    _class_0 = setmetatable({
        __init = function(self, eventLoop, paused)
            ...
            local w, h = 0.5 * Window.w, 0.5 * Window.h
            self.line = {
                [[{\fscx0\fscy0]], [[\alpha&H]], 0, [[&\pos(]],
                ([[%g,%g]]):format(w, h),
                ([[)\an5\bord0%s\p1}]]):format(
                    settings['pause-indicator-background-style']), 0,
                [[{\fscx0\fscy0]], [[\alpha&H]], 0, [[&\pos(]],
                ([[%g,%g]]):format(w, h),
                ([[)\an5\bord0%s\p1}]]):format(
                    settings['pause-indicator-foreground-style']), 0
            }
            if paused then
                self.line[7] =
                ...
                self.line[14] =
                    'm 0 0 m 75 75 m 23 20 l 23 55 33 55 33 20 m 42 20 l 42 55 52 55 52 20'
            else
                self.line[7] =
                ...
                self.line[14] =
                    'm 0 0 m 75 75 m 25.8333 17.18 l 25.8333 57.6 60.8333 37.39'
            end
            AnimationQueue.addAnimation(Animation(0, 1,
...
do
    ...
    local _base_0 = {
        reconfigure = function(self)
            ...
            self.line[2] = ('%g,%g'):format(settings['title-left-margin'],
                                            self.animation.value)
            self.line[3] = ([[)\an7%s%s}]]):format(settings['default-style'],
                                                   settings['title-style'])
            self.animation = Animation(offscreenPos,
        ...
        animate = function(self, value)
            self.line[2] =
                ('%g,%g'):format(settings['title-left-margin'], value)
            self.needsUpdate = true
        end,
        ...
        generateTitleString = function(self, quote)
            if quote == nil then quote = false end
            local title, position, total
            do
                local _obj_0 = self.playlistInfo
                title, position, total = _obj_0['media-title'],
                                         _obj_0['playlist-pos-1'],
                                         _obj_0['playlist-count']
            end
            local prefix =
                (total > 1) and ('%d/%d - '):format(position, total) or ''
            if quote then
    ...
    _class_0 = setmetatable({
        __init = function(self)
            ...
            self.line = {
                ...
                ([[)\an7%s%s}]]):format(settings['default-style'],
                                        settings['title-style']), [[????]]
            }
            self.position = offscreenPos
            self.animation = Animation(offscreenPos,
            ...
            return mp.observe_property('playlist-count', 'number',
                                       updatePlaylistInfo)
        end,
        __base = _base_0,
        __name = "Title",
...
do
    ...
    local _base_0 = {
        reconfigure = function(self)
            ...
            self.line[3] = ([[)\an9%s%s}]]):format(settings['default-style'],
                                                   settings['system-time-style'])
    ...
    _class_0 = setmetatable({
        __init = function(self)
            ...
            self.line = {
                [[{\pos(]], [[-100,0]],
                ([[)\an9%s%s}]]):format(settings['default-style'],
                                        settings['system-time-style']), [[????]]
            }
            self.lastTime = -1
...
local chapters, progressBar, barCache, barBackground, elapsedTime,
      remainingTime, hoverTime
if settings['enable-bar'] then
    ...
    mp.add_key_binding("c", "toggle-inactive-bar",
                       function() return BarBase:toggleInactiveVisibility() end)
end
...
local initDraw
initDraw = function()
    if chapters then chapters:createMarkers() end
    if title then
    ...
    if not (streamMode or duration) then
        ...
        if chapters then bottomZone:removeUIElement(chapters) end
        if hoverTime then hoverTimeZone:removeUIElement(hoverTime) end
        if remainingTime then bottomZone:removeUIElement(remainingTime) end
        streamMode = true
    elseif streamMode and duration then
        ...
        if chapters then bottomZone:addUIElement(chapters) end
        if hoverTime then hoverTimeZone:addUIElement(hoverTime) end
        if remainingTime then bottomZone:addUIElement(remainingTime) end
        streamMode = false
    end
