...
let
  shellsOverlay = self: super: {
    myShells = {
      ...
      buildEnvironmentWithPackages = let
      ...
      in packages:
      super.runCommandCC "shell" { buildInputs = [ my-build-env ] ++ packages; }
      "";
  ...
  myOverlay = self: super: {
    ...
    in rec {
      ...
      idris = super.idrisPackages.with-packages
        (with super.idrisPackages; [ base effects contrib pruviloj lightyear ]);
      ...
      wallabag-client = with super;
        let
          ...
          markdownify = with python3Packages;
...
in { inherit myOverlay shellsOverlay; }
